...
test.describe('G2A Product Search and Cart Test', () => {
  test('Search for product, add to cart, and verify price @search @integration', async ({ browser }) => {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        productPriceBeforeCart = await searchPage.searchAndGetPrice(productName);
        console.log(`Product price before adding to cart: ${productPriceBeforeCart}`);
    ...
    expect(productPriceBeforeCart, 'Product price should not be null').not.toBeNull();
    expect(cartTotalPrice, 'Cart total price should not be null').not.toBeNull();

    if (productPriceBeforeCart && cartTotalPrice) {
      // Remove any non-digit characters and convert to numbers for comparison
      const cleanProductPrice = parseFloat(productPriceBeforeCart.replace(/[^\d.]/g, ''));
      ...
      console.log(`Price comparison successful: Product price "${cleanProductPrice}" matches cart total "${cleanCartPrice}"`);
    } else {
